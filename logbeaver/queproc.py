...
class QueueProcessor (object):
	def __init__ (self, upstream_uri = 'http://localhost:6543', bean_tube = 'logbeaver',
		...
		self.logger = logging.getLogger(__name__)  #to avoid disabling by setup_logging
	...
	def _connect (self, bean_host, bean_port, bean_conn_timeout, bean_tube):
		...
		while True:
			try:
				...
				self.logger.warning("Failed to connect to beanstalkd (%s:%s): %s" % (bean_host, bean_port, e))
	...
	def _send (self, data):
		...
		if resp.status_code != requests.codes.ok:
			self.logger.warning("unsuccessfull response status: %s" % resp.status_code)
			return False
		if resp.text != 'ok':
			self.logger.warning("unsuccessfull response text:%s" % resp.text)
			return False
